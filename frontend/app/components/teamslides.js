...
export default async function Teamslides(){
    ...
    return teammember.map((pers, index) => (
            <div className="teamCard w-full max-h-full relative" key={index}>
                ...
                <div className="teamCardcontent flex flex-col absolute top-0 left-0 origin-center bg-coal-black bg-opacity-50 backdrop-blur-sm p-5 md:px-10 gap-5 w-full h-full transition-all duration-500 max-h-full overflow-scroll">
